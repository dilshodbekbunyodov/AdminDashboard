<template>
  <div :class="showModal ? 'openCatalog menu' : 'defaultCatalog menu'">
    <div class="modal-title">
      <h5>{{ modalValue.id ? 'Редактировать' : 'Добавить' }} тип</h5>
...
const onSubmit = async () => {
  ...
  try {
    if (props.modalValue?.id) {
      await store.dispatch('reference/UPDATE_PRODUCT_TYPE', {
        id: props.modalValue?.id,
        ...payload
      })
    } else {
      await store.dispatch('reference/CREATE_PRODUCT_TYPE', payload)
    }
